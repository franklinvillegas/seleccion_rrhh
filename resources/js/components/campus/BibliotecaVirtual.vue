<template>
    <div class="p-3 pt-5">
        <h4 class="text-color-2 mb-3">Biblioteca Virtual</h4>
        <hr>
        <div class="row justify-content-md-center">
                ...
                <div class="form-group shadow-sm">
                    <div id="accordion">
                        <div class="table-responsive" id="table-media">
                            <vue-good-table :columns="listaLibros.columns" :rows="listaLibros.data" :search-options="{
                        enabled: true,
            ...
            <div class="col-md-7">
                <div class="form-group text-center bg-white" v-if="listaLibros.data[select_content]">
                    <iframe v-if="listaLibros.data[select_content].subido" :src="listaLibros.data[select_content] ? getPDFPath(listaLibros.data[select_content].url) : ''" title="PDF in an i-Frame" frameborder="0" scrolling="auto" height="590" width="100%" allow="fullscreen"></iframe>
                    <iframe v-else :src="listaLibros.data[select_content] ? listaLibros.data[select_content].url : ''" title="PDF in an i-Frame" frameborder="0" scrolling="auto" height="590" width="100%" allow="fullscreen"></iframe>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            listarLibro(id_grado_cur){
                axios.get("api/libro/listar_filtro/"+id_grado_cur+Helper.getFilterURL(this.listaLibros.filtrosBusqueda))
                .then((response) => {
            ...
            listarNivel(){
                axios.get("api/nivel/listar"+Helper.getFilterURL(this.listaLibros.filtrosBusqueda))
                .then((response) => {
                    let data = response.data;
